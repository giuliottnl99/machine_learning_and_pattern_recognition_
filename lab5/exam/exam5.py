...
# def applyPCAToReduce(D, L):
#     for i in range(D.shape[0]): #verify range is fine
#         if i==0 or i==(D.shape[0]-1): #verify if is fine
#             continue
#         PCAReducingMatrix = ut.computePCA_ReducingMatrix(D, L, dim=(i+1))
#         DReduced = PCAReducingMatrix @ D
#         accMLPCA = 
#         print("")
...
def computeMVG_tied_naiveThenPCA(D, L):
    ...
    for i in range(len(arrOfLogs)):
        ...
        accML = ut.computeAccuracyUsingBinaryDivision_MVG(matrix, L, 1, 0, chosenCase='ML')
        print("Using %s the MVG solution error rate is: %f" % (log, ((1-accML)*100)))
        accTied = ut.computeAccuracyUsingBinaryDivision_MVG(matrix, L, 1, 0, chosenCase='tied')
        print("Using %s the tied solution error rate is: %f" % (log, ((1-accTied)*100)))
        accNaive = ut.computeAccuracyUsingBinaryDivision_MVG(matrix, L, 1, 0, chosenCase='naive')
    ...
    accPCAML = ut.computeAccuracyUsingBinaryDivision_MVG(matrixReduced, L, 1, 0, chosenCase='ML')
    print("Using the application of PCA (dim=4) and then MVG have as an error rate of: %f" % (((1-accPCAML)*100)))
    accPCATied = ut.computeAccuracyUsingBinaryDivision_MVG(matrixReduced, L, 1, 0, chosenCase='tied')
    print("Using the application of PCA (dim=4) and then tied MVG have as an error rate of: %f" % (((1-accPCATied)*100)))
    accPCANaive = ut.computeAccuracyUsingBinaryDivision_MVG(matrixReduced, L, 1, 0, chosenCase='naive')
...
if __name__ == '__main__':
    ...
    accML = ut.computeAccuracyUsingBinaryDivision_MVG(D, L, 1, 0, chosenCase='ML')
    print("using MVG model the error rate is: %f" % ((1-accML)*100) )
    #compute error rate using tied
    accTied= ut.computeAccuracyUsingBinaryDivision_MVG(D, L, 1, 0, chosenCase='tied')
    print("using tied Gaussian model the error rate is: %f" % ((1-accTied)*100) )
    #compute error rate using naive
    accNaive = ut.computeAccuracyUsingBinaryDivision_MVG(D, L, 1, 0, chosenCase='naive')
    print("using naive Gaussian model the error rate is: %f" % ((1-accNaive)*100) )
    
    #compute error rate using LDA
    accLDA = ut.doBinaryClassification_PCA_LDA(D, L, chosenMethod='LDA', LValueTrue=1, LValueFalse=0)
    print("accuract using LDA reduction: %f" % ((1-accLDA)*100))
