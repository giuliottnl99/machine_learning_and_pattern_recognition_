...
def applyMVGToComputeLLR_Binary(D, L, chosenCase='ML', PCAdimensions=None):
    (DT, LT), (DV, LV) = divideSamplesRandomly(D, L)
    if PCAdimensions!=None and PCAdimensions<DT.shape[0]:
        pcaReducingMatrix = computePCA_ReducingMatrix(DT, LT, dim=PCAdimensions)
        DT = pcaReducingMatrix @ DT
        DV = pcaReducingMatrix @ DV
    muAndCovDivided = computeMuAndCovForClass(DT, LT, chosenCase=chosenCase)
    #first: compute the likelihoods:
    logScoreMatrix, classNamesArr = computeLogScoreMatrix(DV, LV, muAndCovDivided)
    logPosteriorProbMatrix = computeLogPosterior(logScoreMatrix)
    LLR = logPosteriorProbMatrix[1, :] - logPosteriorProbMatrix[0, :]
    return LLR
...
def effectivePriorRappresentation(prior, Cfn, Cfp):
...
#GMM:
def train_GMM_LBG_EM(X, L, numComponents, covType = 'Full', psiEig = None, epsLLAverage = 1e-6, lbgAlpha = 0.1, verbose=True):
...
def train_GMM_EM_Iteration(X, gmm, covType = 'Full', psiEig = None): 
    ...
    # Compute statistics:
        ...
        Z = gamma.sum()
        F = vcol((vrow_arr(gamma) * X).sum(1)) # Exploit broadcasting to compute the sum
        S = (vrow_arr(gamma) * X) @ X.T
        muUpd = F/Z
